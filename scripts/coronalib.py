...
def profiles(gmean=7.0, gsd=3.4, nday=28):
...
def sim(age, gender, dr, r, gmean=7.0, gsd=3.4, nday=140, burnin=1000,
        ...
        gmean_h=5.9, gsd_h=5.1, prob_h=0.1
        ):
    """Simulate model.
    ...
    delay=profiles(gmean=gmean, gsd=gsd, nday=28)
    
    # Precalculate profile time to icu
    delay_icu=profiles(gmean=mean_days_to_icu,gsd=std_days_to_icu, nday=28)
    
    #Precalculate profile time to hosp
    delay_h=profiles(gmean=gmean_h,gsd=gsd_h, nday=28)
    
    #individual prop hospital
    ind_prob_h = dr/np.mean(dr) * prob_h
    print("Mean prob hospital:" + str(np.mean(ind_prob_h)))
    # individual prob icu
    ...
    for i in range(1, nday):
        ...
        # for infected cases calcualate the probality of hospital admission
        days_infected_lim = np.clip(days_infected, 0, 27)
        ph = delay_h[days_infected_lim] * ind_prob_h
        rans = np.random.random(size=n)

        # only those in state infected can go to hospital
        filt = (rans < ph) & (state[i, ] == 2)
        state[i, filt] = 5

        # calculate the number of days in hospital
        days_h = np.clip(i - firstdayinhospital, 0, 27)
